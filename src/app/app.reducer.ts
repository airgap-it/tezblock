...
export const reducer = createReducer(
  initialState,
  on(
    actions.loadFirstBlockOfCycleSucceeded,
    (state, { firstBlockOfCycle }) => ({
      ...state,
  ...
  on(
    ...
    (
      state,
      { currentVotingPeriod, currentVotingeriodPosition, blocksPerVotingPeriod }
    ) => ({
      ...state,
      currentVotingPeriod,
  ...
  on(
    actions.loadProtocolVariablesSucceeded,
    (state, { protocolVariables }) => ({
      ...state,
      protocolVariables,
      busy: {
        ...state.busy,
...
export const currentCycleSelector = (state: State): number =>
  state.latestBlock ? state.latestBlock.meta_cycle : undefined;
export const currentBlockLevelSelector = (state: State): number =>
  state.latestBlock ? state.latestBlock.level : undefined;
export const cycleStartingBlockLevelSelector = (state: State): number =>
  state.firstBlockOfCurrentCycle
    ? state.firstBlockOfCurrentCycle.level
    : undefined;
export const cycleEndingBlockLevelSelector = (state: State): number =>
  state.firstBlockOfCurrentCycle
    ? state.firstBlockOfCurrentCycle.level +
      (state.protocolVariables.blocks_per_cycle - 1)
    : undefined;
export const cycleProgressSelector = (state: State): number =>
  state.firstBlockOfCurrentCycle && state.latestBlock.level
    ? Math.round(
        ((state.latestBlock.level - state.firstBlockOfCurrentCycle.level) /
          state.protocolVariables.blocks_per_cycle) *
          100
      )
    : undefined;
export const remainingTimeSelector = (state: State): string =>
  state.firstBlockOfCurrentCycle && state.latestBlock.level
    ? getRemainingTime(
        currentBlockLevelSelector(state),
        cycleEndingBlockLevelSelector(state),
        Number(state.protocolVariables.minimal_block_delay)
      )
    : undefined;

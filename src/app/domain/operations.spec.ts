import { OperationErrorsById, OperationError, getTransactionsWithErrors } from './operations'
...
describe('operations', () => {
  describe('getTransactionsWithErrors', () => {
    it('when there is no match between operationErrorsById and transactions then in all transactions set errors to null', () => {
      ...
      const tableState: TableState<any> = { ...getInitialTableState(), data: [transactionA] }
      const operationErrorsById: OperationErrorsById[] = [{ id: 'foo_2', errors: [{ kind: 'foo_kind', id: 'foo_id' }] }]
      const response = getTransactionsWithErrors(operationErrorsById, tableState)
    ...
    it('when there is match between operationErrorsById and transactions then set errors property', () => {
      ...
      const operationErrorsById: OperationErrorsById[] = [{ id: 'foo_1', errors }]
      const response = getTransactionsWithErrors(operationErrorsById, tableState)

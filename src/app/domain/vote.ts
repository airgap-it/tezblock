...
export const fillMissingPeriodTimespans = (
  ...
  const getFromPrevious = (
    index: number,
    array: PeriodTimespan[],
    isStart: boolean,
    originIndex: number
  ): number => {
    if (index < 0) {
    ...
    return {
      start: period.start || getFromPrevious(index - 1, array, true, index),
        ...
        period.end || period.start
          ? fromPeriod(period.start, 1)
          : getFromPrevious(index - 1, array, false, index),
    };
  };
...
export const getPeriodTimespanQuery = (
  period: number,
  direction: Direction
): Body => ({
  fields: ['timestamp'],
...
export const _yayRollsSelector = (divisionOfVotes: DivisionOfVotes[]): number =>
  divisionOfVotes
    ? divisionOfVotes.map((x) => x.max_yay_rolls).reduce((a, b) => a + b, 0)
    : undefined;
export const _nayRollsSelector = (divisionOfVotes: DivisionOfVotes[]): number =>
  divisionOfVotes
    ? divisionOfVotes.map((x) => x.max_nay_rolls).reduce((a, b) => a + b, 0)
    : undefined;
export const _passRollsSelector = (
  divisionOfVotes: DivisionOfVotes[]
): number =>
  divisionOfVotes
    ? divisionOfVotes.map((x) => x.max_pass_rolls).reduce((a, b) => a + b, 0)
    : undefined;
...
export const _yayRollsPercentageSelector = (
  divisionOfVotes: DivisionOfVotes[]
): number =>
  allRollsSelector(divisionOfVotes) > 0
    ? Math.round(
        (_yayRollsSelector(divisionOfVotes) /
          allRollsSelector(divisionOfVotes)) *
          10000
      ) / 100
    : 0;
export const _nayRollsPercentageSelector = (
  divisionOfVotes: DivisionOfVotes[]
): number =>
  allRollsSelector(divisionOfVotes) > 0
    ? Math.round(
        (_nayRollsSelector(divisionOfVotes) /
          allRollsSelector(divisionOfVotes)) *
          10000
      ) / 100
    : 0;

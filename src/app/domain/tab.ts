...
export const compareTabWith = (anotherTabTitle: string) => (tab: Tab) => toLowerCase(tab.title) === toLowerCase(anotherTabTitle)

export const kindToOperationTypes = (kind: KindType): string => (Array.isArray(kind) ? OperationTypes.Ballot : kind)

const proposalsToBallot = (operationCounts: OperationCount[]): OperationCount[] =>
  operationCounts.map(operationCount => ({ ...operationCount, kind: operationCount.kind === 'proposals' ? 'ballot' : operationCount.kind }))
const sumCounts = (aggreagted: { [key: string]: OperationCount[] }): Count[] =>
  Object.keys(aggreagted).map(key => ({
    key,
    count: aggreagted[key].map(aggreagtedItem => parseInt(aggreagtedItem[`count_${aggreagtedItem.field}`], 10)).reduce((a, b) => a + b)
  }))
...
export const updateTabCounts = (tabs: Tab[], counts: Count[]): Tab[] => tabs.map(tab => {

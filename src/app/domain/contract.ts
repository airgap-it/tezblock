...
const networkCondition =
  (tezosNetwork: TezosNetwork) =>
  (tokenContract: TokenContract): boolean => {
    if (!tokenContract.tezosNetwork) {
      return true;
    }

    return tokenContract.tezosNetwork.some(
      (_tezosNetwork) => _tezosNetwork === tezosNetwork
    );
  };
...
export const searchTokenContracts = (
  searchTerm: string,
  tezosNetwork: TezosNetwork
): SearchOptionData[] => {
  if (!searchTerm) {
  ...
  const tokenContractByAddress = getTokenContractByAddress(
    searchTerm,
    tezosNetwork
  );
  ...
  return getTokenContracts(tezosNetwork)
    .data.filter(
      (tokenContract) =>
        tokenContract.name.toLowerCase().indexOf(searchTerm.toLowerCase()) !==
        -1
    )
    .map((tokenContract) => ({
      id: tokenContract.id,
...
export const getTokenContractBy = (
  searchTerm: string,
  tezosNetwork: TezosNetwork
): TokenContract => {
  if (!searchTerm) {
...
export const getTokenContractBySymbol = (
  symbol: string,
  tezosNetwork: TezosNetwork
): TokenContract => {
  if (!symbol) {
  ...
  return getTokenContracts(tezosNetwork).data.find(
    (tokenContract) => tokenContract.symbol === symbol
  );
};

export const getCurrencyConverterPipeArgs = (
  contract: { symbol: string },
  exchangeRates: ExchangeRates
): CurrencyConverterPipeArgs => {
  if (
    isNil(contract) ||
    !isInBTC(contract.symbol) ||
    !get(exchangeRates, `${Currency.BTC}.${Currency.USD}`)
  ) {
    return null;
  }
...
const normalizeParameters = async (
  ...
  try {
    return await faProtocol.normalizeTransactionParameters(
      parameters,
      fallbackEntrypoint
    );
  } catch {
...
export const fillTransferOperations = async (
  ...
  for (const transaction of transactions) {
    ...
    if (
      contract &&
      transaction.kind === OperationTypes.Transaction &&
      transaction.parameters_micheline
    ) {
      const faProtocol = getFaProtocol(
        contract,
        chainNetworkService.getEnvironment(),
        chainNetworkService.getNetwork()
      );
      ...
      const details = await transactionDetailsFromParameters(
        faProtocol,

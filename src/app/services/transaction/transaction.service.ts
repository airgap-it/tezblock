...
export class TransactionService {
  ...
  getAllTransactionsByAddress(
    ...
    const preprocess = (transactions: Transaction[]) =>
      transactions.map((transaction) => ({
        ...
        proposal: transaction.proposal
          ? transaction.proposal.slice(1, transaction.proposal.length - 1)
    ...
    const comparisonOperation =
      kind === 'endorsement' ? Operation.in : Operation.eq;
      ...
      ...[].concat(
        ...fields.map((field, index) => {
          if (field === 'parameters_micheline') {
            ...
            predicates = predicates.concat(
              likeGroup(
                [
                  {
                    field,
                    set: [TezosUtils.encodeAddress(address).toString('hex')],
                  },
                  { field: 'kind', operation: Operation.eq, set: [kind] },
                ],
                `DB${index}`
            ...
            if (supportedTokens) {
              const contractAddresses = supportedTokens
    ...
    if (kind === 'delegation') {
      predicates.push(
        ...andGroup(
          [
            { field: 'delegate', set: [address] },
            { field: 'kind', set: ['origination'] },
            idNotNullPredicate,
          ],
          'B'
        )
    ...
    return this.apiService
      .getTransactionsByPredicates(predicates, limit, orderBy)
      .pipe(
        // map(preprocess),
        switchMap((transactions) => {
          const sources: string[] = transactions.map(
          ...
          if (kind === 'delegation' && sources.length > 0) {
            return this.accountService.getAccountsByIds(sources).pipe(
              map((delegators) =>
                transactions.map((transaction) => {
                  const match = delegators.find(
                    (delegator) => delegator.account_id === transaction.source
                  );
        ...
        switchMap((transactions) => {
          if (kind === 'origination') {
            const originatedSources: string[] = transactions.map(

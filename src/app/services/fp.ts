...
export const isNotEmptyArray = (array: any) =>
  Array.isArray(array) && hasLengthGreaterThanZero(array);
...
export const toNotNilArray = (value: any) =>
  Array.isArray(value) ? value : value ? [value] : [];

export const withoutBraces = (value: string): string =>
  value ? value.replace(squareBrackets, '') : value; //TODO: use pattern.ts
...
export function flatten<T>(value: T[][]): T[] {
  return value.reduce(
    (accumulator, currentItem) => currentItem.concat(accumulator),
    []
  );
}

export const numberToString = (radix?: number) => (value: number) =>
  value.toString(radix);

export const bind =
  (_function: Function, ...args: any[]) =>
  (value: any) =>
    _function.bind(value)(args);

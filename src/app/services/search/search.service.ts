...
export class SearchService {
  ...
  getSearchSources(token: string): Observable<SearchOptionData[]>[] {
    const matchByAccountIds = ['tz', 'KT'].some((prefix) =>
      ...
      result.push(
        this.apiService
          ...
          .pipe(
            map((block) =>
              block
                ? [{ id: block.level.toString(), type: OperationTypes.Block }]
                : []
            )
          )
      );
  ...
  processSearchSelection(option: SearchOptionData) {
    const _option: SearchOptionData = option.type
      ? option
      : { ...option, type: guessType(option.id) };
    ...
    if (
      _option.type === OperationTypes.Account ||
      _option.type === OperationTypes.Baker
    ) {
  ...
  getPreviousSearches(): Observable<SearchOptionData[]> {
    return this.cacheService
      .get<SearchOptionData[]>(CacheKeys.previousSearches)
  ...
  updatePreviousSearches(option: SearchOptionData) {
    ...
    const _option = <SearchOptionData>(
      pick(option, ['id', 'type'].concat(label))
    );

    this.cacheService.update(
      CacheKeys.previousSearches,
      (previousOptions: SearchOptionData[]) =>
        ((previousOptions || []).some(
          (previousOption) => previousOption.id === _option.id
        )
          ? []
          : [_option]
        )

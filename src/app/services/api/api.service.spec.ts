...
describe('ApiService', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      ...
      providers: [
        ...
        {
          provide: ProtocolVariablesService,
          useValue: protocolVariablesServiceMock,
        },
  ...
  describe('getLatestTransactions', () => {
    ...
    it('when Conseil return no transactions of given kind, then does not call getAccountsByIds', () => {
      service
        .getLatestTransactions(10, [OperationTypes.Transaction])
    ...
    it('when Conseil return Origination transactions, then tries add originatedBalance', () => {
      ...
      const mockedTransactions = [
        ...
        {
          kind: OperationTypes.Origination,
          originated_contracts: fakeOriginatedContractName,
        },
      ];
      ...
      const mockedOriginators = [
        { account_id: fakeOriginatedContractName, balance: 6 },
      ];
      const kindList = [OperationTypes.Transaction, OperationTypes.Origination];

      accountServiceMock.getAccountsByIds.and.returnValue(
        of(mockedOriginators)
      );
      proposalServiceMock.addVoteData.and.callFake((transactions, kindList) =>
        of(transactions)
      );

      service.getLatestTransactions(10, kindList).subscribe((transactions) => {
        expect(transactions).toEqual(<any>expectedTransactions);
        expect(accountServiceMock.getAccountsByIds).toHaveBeenCalledWith([
          fakeOriginatedContractName,
        ]);
        expect(proposalServiceMock.addVoteData).toHaveBeenCalledWith(
          <any>expectedTransactions,
          kindList
        );
      });
  ...
  describe('getTransactionsById', () => {
    ...
    it('calls getAccountsByIds with transaction of kind Delegation to set delegatedBalance', () => {
      ...
      const mockedTransactions = [
        ...
        { kind: OperationTypes.Delegation, source: fakeSourceName },
      ];
      const expectedTransactions = [
        { kind: OperationTypes.Transaction },
        {
          kind: OperationTypes.Delegation,
      ...
      proposalServiceMock.addVoteData.and.callFake((transactions) =>
        of(transactions)
      );
    ...
    it('calls getAccountsByIds with transaction of kind Origination to set originatedBalance', () => {
      ...
      const mockedTransactions = [
        ...
        {
          kind: OperationTypes.Origination,
          originated_contracts: fakeOriginatedContractsName,
        },
      ];
      const expectedTransactions = [
        { kind: OperationTypes.Transaction },
      ...
      const mockedOriginations = [
        { account_id: fakeOriginatedContractsName, balance: 8 },
      ];

      accountServiceMock.getAccountsByIds.and.returnValue(
        of(mockedOriginations)
      );
      proposalServiceMock.addVoteData.and.callFake((transactions) =>
        of(transactions)
      );
  ...
  describe('getTransactionsByField', () => {
    ...
    it('calls getAccountsByIds with transaction of kind Delegation to set delegatedBalance', () => {
      ...
      service
        .getTransactionsByField('fake_value', 'fake_field', 'fake_kind', 10)
        .subscribe((transactions) => {
          expect(transactions).toEqual(<any>expectedTransactions);
          expect(accountServiceMock.getAccountsByIds).toHaveBeenCalledWith([
            fakeSourceName,
  ...
  describe('getTransactionsByPredicates', () => {
    ...
    it('calls getAccountsByIds with transaction of kind Delegation to set delegatedBalance', () => {
      ...
      service
        .getTransactionsByPredicates([predicate], 10)
        .subscribe((transactions) => {
          expect(transactions).toEqual(<any>expectedTransactions);
          expect(accountServiceMock.getAccountsByIds).toHaveBeenCalledWith([
            fakeSourceName,
  ...
  describe('getLatestBlocksWithData', () => {
    ...
    it('gets blocks and appends volume, fee and txcount properties', () => {
      ...
      spyOn(service, 'getAdditionalBlockField').and.returnValue(
        of([fakeGetAdditionalBlockFieldObservableResult])
      );
      ...
      service.getLatestBlocksWithData(1).subscribe((blocks) => {
        expect(blocks).toEqual([
          <any>{
            ...
            txcount:
              fakeGetAdditionalBlockFieldObservableResult.count_operation_group_hash,
          },
        ]);
      });
  ...
  describe('getAdditionalBlockField', () => {
    ...
    it('when field is "operation_group_hash" then attach to request body predicate for kind', () => {
      service
        .getAdditionalBlockField([], 'operation_group_hash', 'sum')
        .subscribe(() => {});

...
export class BlockService {
  ...
  getLatest(fields?: string[]): Observable<Block> {
    return this.apiService.getLatestBlocks(1).pipe(
      switchMap((blocks: Block[]) =>
        this.getAdditionalBlockData(blocks, fields)
      ),
      map(first)
    );
  }
  ...
  private getAdditionalBlockData(
    blocks: Block[],
    fields?: string[]
  ): Observable<Block[]> {
    ...
    const isField = (fieldName: string): boolean =>
      fields && fields.some((field) => field === fieldName);
    const getAmounts = isField('volume')
      ? this.getAdditionalBlockField<ConseilAmountSum>(
          blockRange,
          'amount',
          'sum'
        )
      : of<ConseilAmountSum[]>([]);
    const getFees = isField('fee')
      ? this.getAdditionalBlockField<ConseilFeeSum>(blockRange, 'fee', 'sum')
      : of<ConseilFeeSum[]>([]);
    const getCounts = isField('txcount')
      ? this.getAdditionalBlockField<ConseilCount>(
          blockRange,
    ...
    return forkJoin([getAmounts, getFees, getCounts]).pipe(
      map(([amounts, fees, counts]) =>
        blocks.map((block) => {
          const blockAmount = amounts.find(
            (amount: ConseilAmountSum) => amount.block_level === block.level
          );
          const blockFee = fees.find(
            (fee: ConseilFeeSum) => fee.block_level === block.level
          );
          const blockOperations = counts.find(
            (operation: ConseilCount) => operation.block_level === block.level
          );
  ...
  private getAdditionalBlockField<T>(
    blockRange: number[],
    field: string,
    operation: string
  ): Observable<T[]> {
    return this.apiService.getAdditionalBlockField<T>(
      blockRange,
      field,
      operation
    );

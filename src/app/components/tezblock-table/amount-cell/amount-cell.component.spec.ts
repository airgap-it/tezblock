...
describe('AmountCellComponent', () => {
  ...
  describe('currencyAmount$', () => {
    it('when getCurrencyConverterArgs returns undefined then does not emit value', () => {
      cryptoPricesServiceMock.getCurrencyConverterArgs.and.returnValue(of(undefined))
    ...
    it('when data is not truthy then does not emit value', () => {
      cryptoPricesServiceMock.getCurrencyConverterArgs.and.returnValue(of('foo'))
    ...
    it('applies decimals from airgap getProtocolByIdentifier', () => {
      ...
      testScheduler.run(({ expectObservable }) => {
        ...
        expectObservable(component.currencyAmount$).toBe(expected, expectedValues)
      })
    })
  })

...
export class TelegramModalComponent implements OnInit {
  ...
  get botNameInvalid() {
    return (
      this.botNameControl.invalid &&
      (this.botNameControl.dirty || this.botNameControl.touched) &&
      this.botNameControl.errors.maxlength
    );
  }
  ...
  createTelegramBot() {
    if (this.form.valid) {
      ...
      if (botName) {
        window.open(
          `https://t.me/TezosNotifierBot?start=tezblock_${this.botAddress}_${botName}`,
          '_blank'
        );
      }
    }

...
export const transactionToAsset = (transaction: Transaction): Asset => ({
    ...
    this.count$ = this.assets$.pipe(
      map((assets) =>
        Array.isArray(assets)
          ? assets.filter((asset) => isConvertableToUSD(asset.symbol)).length
          : 0
      )
    );
    ...
    this.value$ = combineLatest([
      ...
      switchMap((assets) =>
        get<Asset[]>((_assets) => _assets.length > 0)(assets)
          ? forkJoin(
              assets.map((asset) =>
                isConvertableToUSD(asset.symbol)
                  ? this.cryptoPricesService
                      .getCurrencyConverterArgs(asset.symbol)
                      .pipe(take(1))
                  : of(null)
              )
            ).pipe(

...
export class AppEffects {
  loadLatestBlock$ = createEffect(() =>
    this.actions$.pipe(
      ...
      filter(
        ([action, latestBlock]) =>
          !latestBlock ||
          moment().diff(moment(latestBlock.timestamp), 'seconds') > 60
      ),
      switchMap(() =>
        this.blockService.getLatest().pipe(
          map((latestBlock) =>
            actions.loadLatestBlockSucceeded({ latestBlock })
          ),
  ...
  onCurrentCycleChangeResetCache$ = createEffect(
    () =>
      combineLatest([
        ...
        filter(
          ([currentCycle, cycleCache]) =>
            currentCycle &&
            (!cycleCache ||
              (cycleCache && cycleCache.cycleNumber !== currentCycle))
        ),
        tap(([currentCycle, cycleCache]) => {
          this.cacheService
            .set<CurrentCycleState>(CacheKeys.fromCurrentCycle, {
    ...
    this.actions$.pipe(
      ...
      switchMap(({ cycle }) =>
        ...
        this.baseService
          ...
          .pipe(
            map(first),
            map((firstBlockOfCycle) =>
              actions.loadFirstBlockOfCycleSucceeded({ firstBlockOfCycle })
            ),
            catchError((error) =>
              of(actions.loadFirstBlockOfCycleFailed({ error }))
            )
          )
      )
    )
            ...
            ([
              ...
              actions.loadPeriodInfosSucceeded({
                ...
                blocksPerVotingPeriod:
                  protocolVariables.blocks_per_voting_period,
              })
          ),
        ...
        this.cacheService.get<ExchangeRates>(CacheKeys.exchangeRates).pipe(
          map((currency) => {
            const fiatCurrency = get(
              get(currency, Currency.XTZ),
              fiatCurrencyInfo.currency
            );
            const cryptoCurrency = get(
              get(currency, Currency.XTZ),
              cryptoCurrencyInfo.currency
            );
            const fiatPrice = new BigNumber(fiatCurrency);
            const cryptoPrice = new BigNumber(cryptoCurrency);

            return actions.loadCryptoPriceFromCacheSucceeded({
              fiatPrice,
              cryptoPrice,
            });
          }),
          catchError((error) =>
            of(actions.loadCryptoPriceFromCacheFailed({ error }))
          )
        )
      )
    )
  ...
  onLoadedFromCacheSetCryptoPrices$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map(({ fiatPrice, cryptoPrice }) =>
        actions.loadCryptoPriceSucceeded({ fiatPrice, cryptoPrice })
      )
    )
  );
          ...
          switchMap((exchangeRates) =>
            this.cryptoPricesService
              ...
              .pipe(
                map((prices) => {
                  const fiatCurrency =
                    get(prices, fiatCurrencyInfo.currency) ||
                    get(
                      get(exchangeRates, Currency.XTZ),
                      fiatCurrencyInfo.currency
                    );
                  const cryptoCurrency =
                    get(prices, cryptoCurrencyInfo.currency) ||
                    get(
                      get(exchangeRates, Currency.XTZ),
                      cryptoCurrencyInfo.currency
                    );
                  ...
                  return actions.loadCryptoPriceSucceeded({
                    fiatPrice,
                    cryptoPrice,
                  });
                }),
    ...
    this.actions$.pipe(
      ...
      switchMap((action) =>
        this.cacheService.get<ExchangeRates>(CacheKeys.exchangeRates).pipe(
          switchMap((exchangeRates) =>
            this.cryptoPricesService
              .getCryptoPrices(action.from, [action.to])
              .pipe(
                map((prices) => {
                  const price =
                    get(prices, action.to) ||
                    get(get(exchangeRates, action.from), action.to);

                  return actions.loadExchangeRateSucceeded({
                    from: action.from,
                    to: action.to,
                    price,
                  });
                ...
                catchError((error) => {
                  ...
                  return price
                    ? of(
                        actions.loadExchangeRateSucceeded({
                          from: action.from,
                          to: action.to,
                          price,
                        })
    ...
    this.actions$.pipe(
      ...
      switchMap(() =>
        this.protocolVariablesService.getProtocolVariables().pipe(
          map((protocolVariables) =>
            actions.loadProtocolVariablesSucceeded({ protocolVariables })
          ),
          catchError((error) =>
            of(actions.loadProtocolVariablesFailed({ error }))
          )
        )
      )
    )

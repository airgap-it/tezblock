...
describe('DashboardComponent', () => {
  ...
  describe('priceChartLabels$', () => {
    ...
    const getPriceChartLabels = (toString: (any) => string) =>
      pipe(multiplyBy(1000), numberToDate, bind(toString));
    ...
    it('when pricePeriod is 0 then uses toLocaleTimeString', () => {
      const _getPriceChartLabels = getPriceChartLabels(
      ...
      testScheduler.run(({ expectObservable }) => {
        ...
        const expectedValues = {
          a: [_getPriceChartLabels(_16July2020 / 1000)],
        };

        expectObservable(component.priceChartLabels$).toBe(
          expected,
          expectedValues
        );
      });
  ...
  describe('proposalHash$', () => {
    ...
    it('when active_proposal of latestBlock is null then returns null', () => {
      ...
      testScheduler.run(({ expectObservable }) => {
        ...
        expectObservable(component.proposalHash$).toBe(
          expected,
          expectedValues
        );
      });

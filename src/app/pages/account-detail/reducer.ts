...
const ensure30Days = (balance: Balance[]): Balance[] => {
  ...
  const attachBalance = (date: number): Balance =>
    balance.find(getBalanceFrom(date)) ||
    getPreviousBalance(balance, date) || { balance: 0, asof: date };
...
export const fromBakingBadResponse = (
  ...
  bakingRating:
    response.status === 'success' && response.payoutAccuracy
      ...
      : null,
  tezosBakerFee:
    response.status === 'success' ? extractFee(response.config.fee) : null,
  stakingCapacity: get<number>(
    (stakingCapacity) => stakingCapacity * xtzToMutezConvertionRatio
  )(response.stakingCapacity),
});
...
export const reducer = createReducer(
  ...
  on(
    actions.loadBalanceForLast30DaysSucceeded,
    (state, { balanceFromLast30Days }) => ({
      ...state,

...
export class AccountOverviewComponent extends BaseComponent implements OnInit {
  ...
  public ngOnInit() {
    const loadingAccounts$ = this.store$.select(
      (state) => state.accountsList.accounts.loading
    );
    const accountsData$ = this.store$.select(
      (state) => state.accountsList.accounts.data
    );
    const accountsOrderBy$ = this.store$.select(
      (state) => state.accountsList.accounts.orderBy
    );
    ...
    this.top25ChartOptions$ = this.isMobile$.pipe(
      map(this.getOptions.bind(this))
    );
    this.top25ChartSize$ = this.isMobile$.pipe(
      map((isMobile) =>
        isMobile ? { width: 200, height: 200 } : { width: 800, height: 500 }
      )
    );
      ...
      tooltips: {
        callbacks: {
          label: (tooltipItem: ChartTooltipItem, data: ChartData) => {
            ...
            const balance =
              <number>data.datasets[0].data[tooltipItem.index] * 1000;

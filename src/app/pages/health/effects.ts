    ...
    this.actions$.pipe(
      ...
      switchMap(() =>
        this.healthService.loadLatestNodeBlock().pipe(
          map(latestNodeBlock => actions.loadLatestNodeBlockSucceeded({ latestNodeBlock })),
          catchError(error => of(actions.loadLatestNodeBlockFailed({ error })))
        )
      )
    )
  )
    ...
    this.actions$.pipe(
      ...
      switchMap(() =>
        this.healthService.loadLatestConseilBlock().pipe(
          map(latestConseilBlock => actions.loadLatestConseilBlockSucceeded({ latestConseilBlock })),
          catchError(error => of(actions.loadLatestConseilBlockFailed({ error })))
        )
      )
    )
  )

...
export class HealthComponent implements OnInit {
  ...
  ngOnInit() {
    this.latestNodeBlock$ = this.store$.select(
      (state) => state.health.latestNodeBlock
    );
    this.isNodeUp$ = this.latestNodeBlock$.pipe(
      map(
        (latestNodeBlock) =>
          latestNodeBlock &&
          !isOlderThan4Mins(
            moment(latestNodeBlock.header.timestamp, 'YYYY-MM-DD')
          )
      )
    );
    this.latestConseilBlock$ = this.store$.select(
      (state) => state.health.latestConseilBlock
    );
    this.isConseilUp$ = this.latestConseilBlock$.pipe(
      map(
        (latestConseilBlock) =>
          latestConseilBlock &&
          !isOlderThan4Mins(moment(latestConseilBlock.timestamp, 'YYYY-MM-DD'))
      )
    );

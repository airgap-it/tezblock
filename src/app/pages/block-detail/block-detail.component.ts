...
export class BlockDetailComponent extends BaseComponent implements OnInit {
  ...
  ngOnInit() {
    this.transactionsLoading$ = this.store$.select(
      (state) => state.blockDetails.operations.loading
    );
    this.blockLoading$ = this.store$.select(
      (state) => state.blockDetails.busy.block
    );
    this.transactions$ = this.store$
      .select((state) => state.blockDetails.operations.data)
    ...
    this.orderBy$ = this.store$.select(
      (state) => state.blockDetails.operations.orderBy
    );
    ...
    this.subscriptions.push(
      ...
      // refresh block
      merge(
        this.actions$.pipe(ofType(actions.loadBlockSucceeded)),
        this.actions$.pipe(ofType(actions.loadBlockFailed))
      )
        .pipe(
          delay(refreshRate),
      ...
      merge(
        ...
        .pipe(
          ...
          switchMap(([action, block, kind]) =>
            timer(refreshRate, refreshRate).pipe(map(() => [block.hash, kind]))
          )
        )
        .subscribe(([blockHash, kind]) =>
          this.store$.dispatch(
            actions.loadOperationsByKind({ blockHash, kind })
          )
        ),
    ...
    this.tabs = [
      {
        title: this.translateService.instant(
          'tabbed-table.block-detail.transactions'
        ),
        active: true,
        kind: 'transaction',
        count: undefined,
      ...
      {
        title: this.translateService.instant(
          'tabbed-table.block-detail.delegations'
        ),
        active: false,
        kind: 'delegation',
        count: undefined,
      ...
      {
        title: this.translateService.instant(
          'tabbed-table.block-detail.originations'
        ),
        active: false,
        kind: 'origination',
        count: undefined,
      ...
      {
        title: this.translateService.instant(
          'tabbed-table.block-detail.endorsements'
        ),
        active: false,
        kind: 'endorsement',
        count: undefined,
      ...
      {
        title: this.translateService.instant(
          'tabbed-table.block-detail.activations'
        ),
        active: false,

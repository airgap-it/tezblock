...
export class ProposalDetailEffects {
    ...
    this.actions$.pipe(
      ...
      switchMap(([action, proposalHash, proposal]) => {
        return this.proposalService
          .getMetaVotingPeriods(proposalHash, proposal)
          .pipe(
            map((metaVotingPeriods) =>
              actions.loadMetaVotingPeriodsSucceeded({ metaVotingPeriods })
            ),
            catchError((error) =>
              of(actions.loadMetaVotingPeriodsFailed({ error }))
            )
          );
      })
    )
      ...
      filter(
        ([{ metaVotingPeriods }, periodKind]) =>
          ...
          !!get<MetaVotingPeriod>((period) => period.value)(
            metaVotingPeriods.find(
              (metaVotingPeriod) => metaVotingPeriod.periodKind === periodKind
            )
          )
      ),
      map(([{ metaVotingPeriods }, periodKind]) =>
        actions.loadVotes({ periodKind })
      )
    )
  );
  ...
  loadVotes$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(
        ([{ periodKind }, metaVotingPeriods, proposalHash, pagination]) => {
          return this.proposalService
            .getVotes(periodKind, metaVotingPeriods, proposalHash, pagination)
            .pipe(
  ...
  loadProposalVotes$ = createEffect(() =>
    this.actions$.pipe(
      ...
      filter(
        ([{ periodKind }, proposalHash, pagination]) =>
          periodKind === PeriodKind.Proposal
      ),
    ...
    this.actions$.pipe(
      ...
      switchMap(([action, proposalHash, metaVotingPeriods]) =>
        this.proposalService
          .getVotesTotal(proposalHash, metaVotingPeriods)
          .pipe(
            map((metaVotingPeriods) =>
              actions.loadVotesTotalSucceeded({ metaVotingPeriods })
            ),
        ...
        ({
          ...
          return this.proposalService
            ...
            .pipe(
              ...
              catchError((error) =>
                of(actions.loadPeriodsTimespansFailed({ error }))
              )
            );
        }
      )
    ...
    this.actions$.pipe(
      ...
      switchMap(([action, id]) =>
        this.proposalService.getProposalDescription(id).pipe(
          map((description) =>
            actions.loadProposalDescriptionSucceeded({ description })
          ),
          catchError((error) =>
            of(actions.loadProposalDescriptionFailed({ error }))
          )
        )
      )
    )
  ...
  onLoadVotesLoadDivisionOfVotes$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map(([{ periodKind }, metaVotingPeriods]) => {
        const votingPeriod = get<MetaVotingPeriod>(
          (metaVotingPeriod) => metaVotingPeriod.value
        )(
          metaVotingPeriods.find(
            (metaVotingPeriod) => metaVotingPeriod.periodKind === periodKind
          )
  ...
  loadDivisionOfVotes$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(([{ votingPeriod }, proposalHash]) =>
        this.proposalService
          .getDivisionOfVotes({ proposalHash, votingPeriod })
          .pipe(
            map((divisionOfVotes) =>

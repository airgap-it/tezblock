...
const addOthersStakingBalance = ([top24Bakers, wholeStakingBalance]: [
  Baker[],
  { sum_staking_balance: number }[]
]): Baker[] =>
  top24Bakers.concat({
    ...
    >(
      ...
      (sum_staking_balance) =>
        sum_staking_balance -
        top24Bakers
          .map((baker) => baker.staking_balance)
          .reduce((a, b) => a + b)
    )(wholeStakingBalance),
    block_id: null,
    frozen_balance: null,
    ...
    this.actions$.pipe(
      ...
      switchMap(() =>
        forkJoin(
          ...
          map((top24Bakers) =>
            listActions.loadTop24BakersSucceeded({ top24Bakers })
          ),
          catchError((error) =>
            of(listActions.loadTop24BakersFailed({ error }))
          )
        )
      )
    )

...
export const reducer = createReducer(
  ...
  on(
    actions.loadOperationsCountSucceeded,
    (state, { transferTotal, otherTotal }) => ({
      ...state,
      transferOperations: {
        ...state.transferOperations,
        pagination: {
  ...
  on(
    ...
    (state, { operationErrorsById }) => ({
      ...state,
      transferOperations: <TableState<ContractOperation>>(
        getTransactionsWithErrors(operationErrorsById, state.transferOperations)
      ),
    })
  ),
...
export const transferOperationsSelector = (
  state: State
): ContractOperation[] => {
  const pagination = state.transferOperations.pagination;
  const to = pagination.currentPage * pagination.selectedSize;

  return get<ContractOperation[]>((data) => data.slice(0, to))(
    state.transferOperations.data
  );
};

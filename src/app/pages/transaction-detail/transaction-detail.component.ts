...
import {
  Asset,
  transactionToAsset,
} from '@tezblock/components/assets-value/assets-value.component';
  ...
  implements OnInit
{
  ...
  ngOnInit() {
    ...
    this.transactionsLoading$ = this.store$.select(
      (state) => state.transactionDetails.transactions.loading
    );
    ...
    this.totalAmount$ = this.store$.select(
      (state) => state.transactionDetails.totalAmount
    );
    ...
    this.orderBy$ = this.store$.select(
      (state) => state.transactionDetails.transactions.orderBy
    );
    ...
    this.filteredTransactions$ = combineLatest([
      ...
      map(([transactions, kind]) =>
        transactions.filter((transaction) =>
          Array.isArray(kind)
            ? kind.indexOf(transaction.kind) !== -1
            : transaction.kind === kind
        )
      )
    );

    this.numberOfConfirmations$ = combineLatest([
      this.store$.select((state) => state.transactionDetails.latestBlock),
      this.latestTx$,
    ]).pipe(
      filter(([latestBlock, transaction]) => !!latestBlock),
      map(
        ([latestBlock, transaction]) =>
          latestBlock.level - transaction.block_level
      )
    );
    this.assets$ = this.store$
      ...
      .pipe(
        map((transactions) =>
          (transactions || []).filter(isAsset).map(transactionToAsset)
        )
      );
    ...
    this.subscriptions.push(
      this.activatedRoute.paramMap.subscribe((paramMap) => {
        this.store$.dispatch(
          actions.loadTransactionsByHash({
            transactionHash: paramMap.get('id'),
          })
        );
      ...
      merge(
        ...
        .pipe(
          withLatestFrom(
            this.store$.select(
              (state) => state.transactionDetails.transactionHash
            )
          ),
          switchMap(([action, transactionHash]) =>
            timer(refreshRate, refreshRate).pipe(map(() => transactionHash))
          )
        )
        .subscribe((transactionHash) =>
          this.store$.dispatch(
            actions.loadTransactionsByHash({ transactionHash })
          )
        ),
      ...
      this.store$
        ...
        .subscribe(
          (counts) => (this.tabs = updateTabCounts(this.tabs, counts))
        ),
      ...
      this.filteredTransactions$
        .pipe(
          filter((transactions) =>
            transactions.some((transaction) => isInBTC(transaction.symbol))
          ),
          switchMap(() =>
            getRefresh([
    ...
    this.titleService.setTitle(
      `Operation ${this.aliasService.getFormattedAddress(
    ...
    this.tabs = [
      {
        title: this.translateService.instant(
          'tabbed-table.transaction-detail.transactions'
        ),
        active: true,
        kind: 'transaction',
        count: undefined,
      ...
      {
        title: this.translateService.instant(
          'tabbed-table.transaction-detail.delegations'
        ),
        active: false,
        kind: 'delegation',
        count: undefined,
      ...
      {
        title: this.translateService.instant(
          'tabbed-table.transaction-detail.originations'
        ),
        active: false,
        kind: 'origination',
        count: undefined,
      ...
      {
        title: this.translateService.instant(
          'tabbed-table.transaction-detail.reveals'
        ),
        active: false,
        kind: 'reveal',
        count: undefined,
        icon: this.iconPipe.transform('eye'),
        columns: columns[OperationTypes.Reveal](
          { pageId, showFiatValue },
          this.translateService
        ),
        disabled: function () {
      ...
      {
        title: this.translateService.instant(
          'tabbed-table.transaction-detail.activations'
        ),
        active: false,
        kind: 'activate_account',
        count: undefined,
        icon: this.iconPipe.transform('handHoldingSeedling'),
        columns: columns[OperationTypes.Activation](
          { pageId, showFiatValue },
          this.translateService
        ),
        disabled: function () {
      ...
      {
        title: this.translateService.instant(
          'tabbed-table.transaction-detail.votes'
        ),
        active: false,

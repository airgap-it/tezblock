...
export class TransactionDetailEffects {
  ...
  getTransactions$ = createEffect(() =>
    this.actions$.pipe(
      ...
      withLatestFrom(
        this.store$.select(
          (state) => state.transactionDetails.transactions.pagination
        ),
        this.store$.select(
      ...
      switchMap(([{ transactionHash }, pagination, orderBy]) =>
        this.apiService
          ...
          .pipe(
            ...
            catchError((error) =>
              of(actions.loadTransactionsByHashFailed({ error }))
            )
          )
      )
    )
  ...
  onLoadTransactionsByHashLoadTotalAmountAndFee$ = createEffect(() =>
    this.actions$.pipe(
      ofType(actions.loadTransactionsByHash),
      switchMap(() => [
        actions.loadTransactionsTotalAmount() /*, actions.loadTransactionsTotalFee()*/,
      ])
    )
  );
      ...
      switchMap(([action, address]) =>
        this.baseService
          ...
          .pipe(
            ...
            map((totalAmount) =>
              actions.loadTransactionsTotalAmountSucceeded({ totalAmount })
            ),
            catchError((error) =>
              of(actions.loadTransactionsTotalAmountFailed({ error }))
            )
          )
      )
    )
      ...
      switchMap(([action, address]) =>
        this.baseService
          ...
          .pipe(
            ...
            map((totalFee) =>
              actions.loadTransactionsTotalFeeSucceeded({ totalFee })
            ),
            catchError((error) =>
              of(actions.loadTransactionsTotalFeeFailed({ error }))
            )
          )
      )
    )

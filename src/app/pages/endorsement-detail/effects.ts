...
export class EndorsementDetailEffects {
  onUrlChangeGetEndorsementDetails$ = createEffect(() =>
    this.actions$.pipe(
      ...
      withLatestFrom(
        this.store$.select((state) => state.endorsementDetails.endorsements)
      ),
      ...
      switchMap(({ id }) =>
        this.apiService.getEndorsementsById(id, 1).pipe(
          map((endorsements: Transaction[]) =>
            EndorsementDetailActions.loadEndorsementDetailsSucceeded({
              endorsement: first(endorsements),
            })
          ),
          catchError((error) =>
            of(EndorsementDetailActions.loadEndorsementDetailsFailed({ error }))
          )
        )
      )
    )
  ...
  onUrlChangeSelectEndorsement$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map(([action, endorsements]) =>
        EndorsementDetailActions.slotSelected({
          operation_group_hash: action.id,
        })
      )
    )
  );
    ...
    this.actions$.pipe(
      ...
      switchMap(([action, endorsement]) =>
        this.apiService
          ...
          .pipe(
            map((endorsements: Transaction[]) =>
              EndorsementDetailActions.loadEndorsementsSucceeded({
                endorsements,
              })
            ),
            catchError((error) =>
              of(EndorsementDetailActions.loadEndorsementsFailed({ error }))
            )
          )
      )
    )

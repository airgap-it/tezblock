...
export const timestampsToCountsPerDay = (timestamps: number[]): number[] => {
  const diffsInDays = timestamps.map((timestamp) =>
    moment().diff(moment(timestamp), 'days')
  );

  return range(0, noOfDays).map(
    (index) => diffsInDays.filter((diffsInDay) => diffsInDay === index).length
  );
};

export const toAmountPerDay = (
  data: actions.TransactionChartItem[],
  network: TezosNetwork
): number[] => {
  const toDiffsInDays = (
    data: actions.TransactionChartItem[]
  ): { diffInDays: number; amount: number }[] =>
    data.map((item) => ({
      diffInDays: moment().diff(moment(item.timestamp), 'days'),
      amount: item.amount,
    }));
  const sum = (data: {
    [key: string]: { diffInDays: number; amount: number }[];
  }): number[] =>
    range(0, noOfDays).map((index) => {
      const match = data[index];

      return match
        ? match.map((item) => item.amount).reduce((a, b) => a + b)
        : 0;
    });
  const decimals = getDecimalsForSymbol('xtz', network);
  const amountToXTZ = (data: number[]): number[] =>
    data.map((item) => toXTZ(item, decimals) / 1000);

  return pipe(toDiffsInDays, groupBy('diffInDays'), sum, amountToXTZ)(data);
};

export const toTransactionsChartDataSource =
  (countLabel: string, amountLabel: string, network: TezosNetwork) =>
  (data: actions.TransactionChartItem[]): { data: number[]; label: string }[] =>
    [
      {
        data: timestampsToCountsPerDay(data.map((item) => item.timestamp)),
        label: countLabel,
      },
      { data: toAmountPerDay(data, network), label: amountLabel },
    ];
const timestampsToChartDataSource =
  (label: string) =>
  (timestamps: number[]): { data: number[]; label: string } => ({
...
export class ListComponent extends BaseComponent implements OnInit {
  ...
  readonly chartLabels: string[] = range(0, noOfDays).map((index) =>
    moment().add(-index, 'days').format('DD.MM.YYYY')
  );
  ...
  ngOnInit() {
    this.routeName$ = this.activatedRoute.paramMap.pipe(
      map((paramMap) => paramMap.get('route'))
    );
    this.activatedRoute.paramMap.subscribe((paramMap) => {
      this.titleService.setTitle(
        `Tezos ${paramMap
          .get('route')
          .replace(/^\w/, (c) => c.toUpperCase())}s - tezblock`
      );
    });
    this.routeName$.subscribe((routeName) => {
      try {
        switch (routeName) {
          case 'block':
            const blockLoading$ = this.store$.select(
              (state) => state.list.blocks.loading
            );
            const blockData$ = this.store$.select(
              (state) => state.list.blocks.data
            );
            const blockOrderBy$ = this.store$.select(
              (state) => state.list.blocks.orderBy
            );
          ...
          case 'transaction':
              ...
              tooltips: {
                callbacks: {
                  label: (tooltipItem: ChartTooltipItem, data: ChartData) => {
                    ...
                    const valueLabel =
                      tooltipItem.datasetIndex === 0 ? value : `${value}K ꜩ`;
            ...
            this.subscriptions.push(
              getRefresh([
                this.actions$.pipe(
                  ofType(actions.loadTransactionsCountLast24hSucceeded)
                ),
                this.actions$.pipe(
                  ofType(actions.loadTransactionsCountLast24hFailed)
                ),
              ]).subscribe(() =>
                this.store$.dispatch(actions.loadTransactionsCountLast24h())
              ),
              getRefresh([
                this.actions$.pipe(
                  ofType(actions.loadTransactionsChartDataSucceeded)
                ),
                this.actions$.pipe(
                  ofType(actions.loadTransactionsChartDataFailed)
                ),
              ]).subscribe(() =>
                this.store$.dispatch(actions.loadTransactionsChartData())
              )
            );
            this.transactionsCountLast24h$ = this.store$.select(
              (state) => state.list.transactionsCountLast24h
            );
            ...
            const transactionLoading$ = this.store$.select(
              (state) => state.list.transactions.loading
            );
            const transactionData$ = this.store$.select(
              (state) => state.list.transactions.data
            );
            const transactionOrderBy$ = this.store$.select(
              (state) => state.list.transactions.orderBy
            );
            this.subscriptions.push(
              getRefresh([
                ...
                this.store$.dispatch(actions.loadTransactions())
              )
            );
            this.setupTable(
              columns[OperationTypes.Transaction](
                { showFiatValue: this.isMainnet },
                this.translateService
              ),
              transactionData$,
          ...
          case 'activation':
            this.subscriptions.push(
              getRefresh([
                this.actions$.pipe(
                  ofType(actions.loadActivationsCountLast24hSucceeded)
                ),
                this.actions$.pipe(
                  ofType(actions.loadActivationsCountLast24hFailed)
                ),
              ]).subscribe(() =>
                this.store$.dispatch(actions.loadActivationsCountLast24h())
              ),
              getRefresh([
                this.actions$.pipe(
                  ofType(actions.loadActivationsCountLastXdSucceeded)
                ),
                this.actions$.pipe(
                  ofType(actions.loadActivationsCountLastXdFailed)
                ),
              ]).subscribe(() =>
                this.store$.dispatch(actions.loadActivationsCountLastXd())
              )
            );
            this.activationsCountLast24h$ = this.store$.select(
              (state) => state.list.activationsCountLast24h
            );
            ...
            const activationsLoading$ = this.store$.select(
              (state) => state.list.activations.loading
            );
            const activationsData$ = this.store$.select(
              (state) => state.list.activations.data
            );
            const activationsOrderBy$ = this.store$.select(
              (state) => state.list.activations.orderBy
            );
            this.subscriptions.push(
              getRefresh([
                ...
                this.store$.dispatch(actions.loadActivations())
              )
            );
            this.setupTable(
              columns[OperationTypes.Activation](
                { showFiatValue: this.isMainnet },
                this.translateService
              ),
              activationsData$,
          ...
          case 'origination':
            this.subscriptions.push(
              getRefresh([
                this.actions$.pipe(
                  ofType(actions.loadOriginationsCountLast24hSucceeded)
                ),
                this.actions$.pipe(
                  ofType(actions.loadOriginationsCountLast24hFailed)
                ),
              ]).subscribe(() =>
                this.store$.dispatch(actions.loadOriginationsCountLast24h())
              ),
              getRefresh([
                this.actions$.pipe(
                  ofType(actions.loadOriginationsCountLastXdSucceeded)
                ),
                this.actions$.pipe(
                  ofType(actions.loadOriginationsCountLastXdFailed)
                ),
              ]).subscribe(() =>
                this.store$.dispatch(actions.loadOriginationsCountLastXd())
              )
            );
            this.originationsCountLast24h$ = this.store$.select(
              (state) => state.list.originationsCountLast24h
            );
            ...
            const originationsLoading$ = this.store$.select(
              (state) => state.list.originations.loading
            );
            const originationsData$ = this.store$.select(
              (state) => state.list.originations.data
            );
            const originationsOrderBy$ = this.store$.select(
              (state) => state.list.originations.orderBy
            );
            ...
            this.subscriptions.push(
              getRefresh([
                ...
                this.store$.dispatch(actions.loadOriginations())
              )
            );
            this.setupTable(
              columns[OperationTypes.Origination](
                { showFiatValue: this.isMainnet },
                this.translateService
              ),
              originationsData$,
            ...
            break;
          case 'delegation':
            const delegationsLoading$ = this.store$.select(
              (state) => state.list.delegations.loading
            );
            const delegationsData$ = this.store$.select(
              (state) => state.list.delegations.data
            );
            const delegationsOrderBy$ = this.store$.select(
              (state) => state.list.delegations.orderBy
            );
            ...
            this.subscriptions.push(
              getRefresh([
                ...
                this.store$.dispatch(actions.loadDelegations())
              )
            );
            this.setupTable(
              columns[OperationTypes.Delegation](
                { showFiatValue: this.isMainnet },
                this.translateService
              ),
              delegationsData$,
            ...
            break;
          case 'endorsement':
            const endorsementsLoading$ = this.store$.select(
              (state) => state.list.endorsements.loading
            );
            const endorsementsData$ = this.store$.select(
              (state) => state.list.endorsements.data
            );
            const endorsementsOrderBy$ = this.store$.select(
              (state) => state.list.endorsements.orderBy
            );
            ...
            this.setupTable(
              columns[OperationTypes.Endorsement](
                { showFiatValue: this.isMainnet },
                this.translateService
              ),
              endorsementsData$,
            ...
            break;
          case 'vote':
            const votesLoading$ = this.store$.select(
              (state) => state.list.votes.loading
            );
            const votesData$ = this.store$.select(
              (state) => state.list.votes.data
            );
            const votesOrderBy$ = this.store$.select(
              (state) => state.list.votes.orderBy
            );
            ...
            break;
          case 'double-baking':
            const dbLoading$ = this.store$.select(
              (state) => state.list.doubleBakings.loading
            );
            const dbData$ = this.store$.select(
              (state) => state.list.doubleBakings.data
            );
            const dbOrderBy$ = this.store$.select(
              (state) => state.list.doubleBakings.orderBy
            );
            ...
            this.subscriptions.push(
              getRefresh([
                ...
                this.store$.dispatch(actions.loadDoubleBakings())
              )
            );
            this.setupTable(
              columns[OperationTypes.DoubleBakingEvidenceOverview](
                { showFiatValue: this.isMainnet },
                this.translateService
              ),
              dbData$,
          ...
          case 'double-endorsement':
            const deLoading$ = this.store$.select(
              (state) => state.list.doubleEndorsements.loading
            );
            const deData$ = this.store$.select(
              (state) => state.list.doubleEndorsements.data
            );
            const deOrderBy$ = this.store$.select(
              (state) => state.list.doubleEndorsements.orderBy
            );

            this.subscriptions.push(
              getRefresh([
                this.actions$.pipe(
                  ofType(actions.loadDoubleEndorsementsFailed)
                ),
                this.actions$.pipe(
                  ofType(actions.loadDoubleEndorsementsSucceeded)
                ),
              ]).subscribe(() =>
                this.store$.dispatch(actions.loadDoubleEndorsements())
              )
            );
            this.setupTable(
            ...
            break;
          case 'bakers':
            const bakersLoading$ = this.store$.select(
              (state) => state.list.activeBakers.loading
            );
            const bakersData$ = this.store$.select(
              (state) => state.list.activeBakers.data
            );
            const bakersOrderBy$ = this.store$.select(
              (state) => state.list.activeBakers.orderBy
            );
          ...
          case 'proposal':
              ...
              .pipe(
                map(
                  (proposals) =>
                    ...
                    proposals.pagination.currentPage *
                      proposals.pagination.selectedSize ===
                      proposals.data.length
                )
              );
            const proposalLoading$ = this.store$.select(
              (state) => state.list.proposals.loading
            );
            const proposalData$ = this.store$.select(
              (state) => state.list.proposals.data
            );
            const proposalOrderBy$ = this.store$.select(
              (state) => state.list.proposals.orderBy
            );
            ...
            this.setupTable(
              columns[OperationTypes.ProposalOverview](
                { showFiatValue: this.isMainnet },
                this.translateService
              ),
              proposalData$,
            ...
            break;
          case 'contract':
            const loadingContracts$ = this.store$.select(
              (state) => state.list.contracts.loading
            );
            const contractsData$ = this.store$.select(
              (state) => state.list.contracts.data
            );
            const contractsOrderBy$ = this.store$.select(
              (state) => state.list.contracts.orderBy
            );

            this.store$.dispatch(actions.loadContracts());

            this.setupTable(
              columns[OperationTypes.Contract](
                { showFiatValue: this.isMainnet },
                this.translateService
              ),
              contractsData$,

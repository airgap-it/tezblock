...
const getTimestamp24hAgo = (): number => moment().add(-1, 'days').valueOf();

const getTimestamp7dAgo = (): number => moment().add(-7, 'days').valueOf();
...
export class ListEffects {
    ...
    this.actions$.pipe(
      ...
      switchMap(([action, pagination, orderBy]) => {
        return this.apiService
          ...
          .pipe(
            map((blocks: Block[]) =>
              listActions.loadBlocksSucceeded({ blocks })
            ),
        ...
        return this.apiService
          ...
          .pipe(
            map((transactions: Transaction[]) =>
              listActions.loadTransactionsSucceeded({ transactions })
            ),
            catchError((error) =>
              of(listActions.loadTransactionsFailed({ error }))
            )
          );
      })
    )
    ...
    this.actions$.pipe(
      ...
      switchMap(([action, pagination, orderBy]) => {
        return this.apiService
          ...
          .pipe(
            map((transactions: Transaction[]) =>
              listActions.loadActivationsSucceeded({ transactions })
            ),
            catchError((error) =>
              of(listActions.loadActivationsFailed({ error }))
            )
          );
      })
    )
    ...
    this.actions$.pipe(
      ...
      switchMap(([action, pagination, orderBy]) => {
        return this.apiService
          ...
          .pipe(
            map((transactions: Transaction[]) =>
              listActions.loadOriginationsSucceeded({ transactions })
            ),
            catchError((error) =>
              of(listActions.loadOriginationsFailed({ error }))
            )
          );
      })
    )
    ...
    this.actions$.pipe(
      ...
      switchMap(([action, pagination, orderBy]) => {
        return this.apiService
          ...
          .pipe(
            map((transactions: Transaction[]) =>
              listActions.loadDelegationsSucceeded({ transactions })
            ),
            catchError((error) =>
              of(listActions.loadDelegationsFailed({ error }))
            )
          );
      })
    )
      ...
      switchMap(([action, pagination, orderBy]) => {
        return this.apiService
          ...
          .pipe(
            ...
            catchError((error) =>
              of(listActions.loadDoubleBakingsFailed({ error }))
            )
          );
      })
    )
  ...
  getDoubleBakingsBlockData$ = createEffect(() =>
    this.actions$.pipe(
      ...
      withLatestFrom(
        this.store$.select((state) => state.list.doubleBakings.temporaryData)
      ),
      switchMap(([action, temporaryData]) => {
        return combineLatest(
          temporaryData.map((doubleBaking) => {
            return this.rewardService
              .getDoubleBakingEvidenceData(
                doubleBaking.block_level,
                doubleBaking.operation_group_hash
              )
              .pipe(
                map((additionalData) => {
                  return {
      ...
      switchMap(([action, pagination, orderBy]) => {
        return this.apiService
          ...
          .pipe(
            ...
            catchError((error) =>
              of(listActions.loadDoubleEndorsementsFailed({ error }))
            )
          );
      })
    )
  ...
  getDoubleEndorsementsBlockData$ = createEffect(() =>
    this.actions$.pipe(
      ...
      withLatestFrom(
        this.store$.select(
          (state) => state.list.doubleEndorsements.temporaryData
        )
      ),
      switchMap(([action, temporaryData]) => {
        return combineLatest(
          temporaryData.map((doubleEndorsement) => {
            return this.rewardService
              .getDoubleEndorsingEvidenceData(
                doubleEndorsement.block_level,
                doubleEndorsement.operation_group_hash
              )
              .pipe(
                map((additionalData) => {
                  return {
          ...
          map((doubleEndorsements) => {
            return listActions.loadDoubleEndorsementsSucceeded({
              doubleEndorsements,
            });
          }),
          catchError((error) =>
            ...
            switchMap((activeBakers) =>
              this.apiService
                .getNumberOfDelegatorsByBakers(
                  activeBakers.map((activeBaker) => activeBaker.pkh)
                )
                .pipe(
                  map((numberOfDelegatorsByBakers) =>
                    listActions.loadActiveBakersSucceeded({
                      activeBakers: activeBakers.map((activeBaker) => {
                        const match = numberOfDelegatorsByBakers.find(
                          (item) => item.delegate_value == activeBaker.pkh
                        );
                        const number_of_delegators = match
                          ? match.number_of_delegators
                          : null;
                  ...
                  catchError((error) =>
                    of(listActions.loadActiveBakersFailed({ error }))
                  )
                )
            )
          );
    ...
    this.actions$.pipe(
      ...
      switchMap(() =>
        this.apiService.getTotalBakersAtTheLatestBlock().pipe(
          map((totalActiveBakers) =>
            listActions.loadTotalActiveBakersSucceeded({ totalActiveBakers })
          ),
          catchError((error) =>
            of(listActions.loadTotalActiveBakersFailed({ error }))
          )
        )
      )
    )
    ...
    this.actions$.pipe(
      ...
      switchMap(([action, pagination, orderBy]) => {
        return this.apiService
          ...
          .pipe(
            map((proposals) =>
              listActions.loadProposalsSucceeded({ proposals })
            ),
            catchError((error) =>
              of(listActions.loadProposalsFailed({ error }))
            )
          );
      })
    )
    ...
    this.actions$.pipe(
      ...
      switchMap(([action, pagination, orderBy]) => {
        return this.apiService
          ...
          .pipe(
            map((endorsements: Transaction[]) =>
              listActions.loadEndorsementsSucceeded({ endorsements })
            ),
            catchError((error) =>
              of(listActions.loadEndorsementsFailed({ error }))
            )
          );
      })
    )
    ...
    this.actions$.pipe(
      ...
      switchMap(() =>
        this.getEntitiesSince(getTimestamp24hAgo(), 'activate_account').pipe(
          ...
          map((activationsCountLast24h) =>
            listActions.loadActivationsCountLast24hSucceeded({
              activationsCountLast24h,
            })
          ),
          catchError((error) =>
            of(listActions.loadActivationsCountLast24hFailed({ error }))
          )
        )
      )
    )
    ...
    this.actions$.pipe(
      ...
      switchMap(() =>
        this.getEntitiesSince(getTimestamp24hAgo(), 'origination').pipe(
          ...
          map((originationsCountLast24h) =>
            listActions.loadOriginationsCountLast24hSucceeded({
              originationsCountLast24h,
            })
          ),
          catchError((error) =>
            of(listActions.loadOriginationsCountLast24hFailed({ error }))
          )
        )
      )
    )
    ...
    this.actions$.pipe(
      ...
      switchMap(() =>
        this.getEntitiesSince(getTimestamp24hAgo(), 'transaction').pipe(
          ...
          map((transactionsCountLast24h) =>
            listActions.loadTransactionsCountLast24hSucceeded({
              transactionsCountLast24h,
            })
          ),
          catchError((error) =>
            of(listActions.loadTransactionsCountLast24hFailed({ error }))
          )
        )
      )
    )
    ...
    this.actions$.pipe(
      ...
      switchMap(() =>
        this.getEntitiesSince(getTimestamp7dAgo(), 'activate_account').pipe(
          map((activations) =>
            activations.map((activation) => activation.timestamp)
          ),
          map((activationsCountLastXd) =>
            listActions.loadActivationsCountLastXdSucceeded({
              activationsCountLastXd,
            })
          ),
          catchError((error) =>
            of(listActions.loadActivationsCountLastXdFailed({ error }))
          )
        )
      )
    )
    ...
    this.actions$.pipe(
      ...
      switchMap(() =>
        this.getEntitiesSince(getTimestamp7dAgo(), 'origination').pipe(
          map((originations) =>
            originations.map((origination) => origination.timestamp)
          ),
          map((originationsCountLastXd) =>
            listActions.loadOriginationsCountLastXdSucceeded({
              originationsCountLastXd,
            })
          ),
          catchError((error) =>
            of(listActions.loadOriginationsCountLastXdFailed({ error }))
          )
        )
      )
    )
  ...
  loadTransactionsCountLastXd$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(() =>
        this.getEntitiesSince(getTimestamp7dAgo(), 'transaction', [
          'timestamp',
          'amount',
        ]).pipe(
          map((transactionsChartData) =>
            listActions.loadTransactionsChartDataSucceeded({
              transactionsChartData,
            })
          ),
          catchError((error) =>
            of(listActions.loadTransactionsChartDataFailed({ error }))
          )
        )
      )
    )
  ...
  loadContracts$ = createEffect(() =>
    this.actions$.pipe(
      ...
      withLatestFrom(
        ...
        this.store$
          .select((state) => state.list.contracts.orderBy)
          .pipe(map(toNotNilArray))
      ),
      switchMap(([action, pagination, orderBy]) =>
        this.baseService
          ...
          .pipe(
            map((contracts) =>
              listActions.loadContractsSucceeded({ contracts })
            ),
            catchError((error) =>
              of(listActions.loadContractsFailed({ error }))
            )
          )
      )
    )
  ...
  onLoadTransactionsSucceededLoadErrors$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map((action) => [
        action[Object.keys(action).filter((key) => key !== 'type')[0]],
        action.type,
      ]),
      filter(([transactions, actionType]) =>
        transactions.some((transaction) => transaction.status !== 'applied')
      ),
      map(([transactions, actionType]) =>
        listActions.loadTransactionsErrors({ transactions, actionType })
      )
    )
  );
      ...
      switchMap(({ transactions, actionType }) =>
        this.apiService.getErrorsForOperations(transactions).pipe(
          map((operationErrorsById) =>
            listActions.loadTransactionsErrorsSucceeded({
              operationErrorsById,
              actionType,
            })
          ),
          catchError((error) =>
            of(listActions.loadTransactionsErrorsFailed({ error }))
          )
        )
      )
    )

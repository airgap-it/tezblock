...
describe('ListComponent', () => {
  ...
  describe('timestampsToCountsPerDay', () => {
    it('counts only dates from last 7 days', () => {
      const _12daysAgo = moment().add(-12, 'days').valueOf();
      const _8daysAgo = moment().add(-8, 'days').valueOf();
      const _6daysAgo = moment().add(-6, 'days').valueOf();
      const _3daysAgo = moment().add(-3, 'days').valueOf();
  ...
  describe('toAmountPerDay', () => {
    it('sum amounts for each of last 7 days converting to xtz', () => {
      ...
      const input: TransactionChartItem[] = [
        {
          timestamp: moment().add(-12, 'days').valueOf(),
          amount: 1 * Math.pow(10, shift),
        },
        {
          timestamp: moment().add(-3, 'days').valueOf(),
          amount: 5 * Math.pow(10, shift),
        },
      ];

      expect(toAmountPerDay(input, TezosNetwork.MAINNET)).toEqual([
        0, 0, 0, 5, 0, 0, 0,
      ]);
    });
  });

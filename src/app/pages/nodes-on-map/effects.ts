...
export class NodesOnMapEffects {
  loadConnectedNodes$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap((orderBy) =>
        this.nodeService
          .loadConnectedNodes(orderBy)
          .pipe(map((nodes: any) => nodes as Node[]))
          .pipe(
            map((data) => actions.loadConnectedNodesSucceeded({ data })),
            catchError((error) =>
              of(actions.loadConnectedNodesFailed({ error }))
            )
          )
      )
    )
    ...
    this.actions$.pipe(
      ...
      switchMap(() =>
        this.nodeService.loadConnectedNodesPerCountry().pipe(
          ...
          catchError((error) =>
            of(actions.loadConnectedNodesPerCountryFailed({ error }))
          )
        )
      )
    )

...
export const reducer = createReducer(
  ...
  on(actions.loadMoreConnectedNodes, state => {
    ...
    const dataTotal: number = get<Node[]>(_data => _data.length)(state.connectedNodes.data)
    const dataTotalPages: number = get<number>(total => Math.ceil(total / state.connectedNodes.pagination.selectedSize))(dataTotal)
  ...
  on(actions.loadConnectedNodesPerCountrySucceeded, (state, { connectedNodesPerCountry }) => ({
    ...state,

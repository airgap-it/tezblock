import { ChangeDetectorRef, Component, OnInit, AfterViewInit, ElementRef, Renderer2, ViewChild } from '@angular/core'
...
export class NodesOnMapComponent extends BaseComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.loading$ = this.store$.select((state) => state.connectedNodes.connectedNodes.loading)
    this.showLoadMore$ = this.store$.select(fromRoot.connectedNodes.showLoadMore)
    this.connectedNodesPerCountry$ = this.store$.select((state) => state.connectedNodes.connectedNodesPerCountry)
    this.subscriptions.push(combineLatest([this.isMapReady$, this.connectedNodesPerCountry$]).subscribe(this.onMapDataReady.bind(this)))
    this.orderBy$ = this.store$.select((state) => state.connectedNodes.connectedNodes.orderBy)
  }
  ...
  ngAfterViewInit() {
    ...
    objElm.onload = () => {
      const paths: NodeListOf<SVGPathElement> = objElm.contentDocument.querySelectorAll('path')
  ...
  private onMapDataReady([pathList, connectedNodesPerCountry]: [NodeListOf<SVGPathElement>, Heatmap[]]) {
    ...
    const areNodesForCountry = (path: SVGPathElement): boolean => {
      return connectedNodesPerCountry.some((node) => node.countryCode === path.id)
    }
    const attachPopup = (svgPathElement: SVGPathElement) => {
      svgPathElement.onmouseenter = (e: MouseEvent) => {
        ...
        this.selectedCountry = connectedNodesPerCountry.find((node) => node.countryCode === path.id)
        this.changeDetectorRef.detectChanges()
        path.setAttribute('style', `fill: ${tinycolor2(countryColors[path.id]).darken(15)};fill-rule:${fillRule}`)
      ...
      svgPathElement.onmouseleave = (e: MouseEvent) => {
        svgPathElement.setAttribute('style', `fill: ${countryColors[svgPathElement.id]};fill-rule:${fillRule}`)
